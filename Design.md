---
name: Resume Builder
description: A clear workspace for reviewing experience and preparing applications.
colors:
  primary: "#176B5B"
...
2026-09-10. Owner approved the shared Module 1/2 visual concept and authorized implementation. Shared shell and Module 1 refresh are being verified; Module 2 is not yet implemented. All current feature categories stay in scope. Build each vertical module's UI with its API and tests; do not postpone all styling until the last day.

## Product feel

A calm professional workspace: warm neutral background, white editing surfaces, dark readable type and one deep teal accent. The resume is the central object; actions and status are clear without large decorative hero panels. Keep details available through tabs, expandable analysis and a side panel. Motion communicates state changes and never hides controls.

## Information architecture

Desktop sidebar: Overview, Resumes, Match a role, Resume builder, Find jobs, Match history. Settings and Sign out sit at the bottom. Preserve all current labels and URLs. On mobile use a labelled menu, compact top bar and single-column content; never squeeze desktop navigation into 768px.
...
|---|---|
| Canvas / surface / subtle | #F5F1E8 / #FFFFFF / #FAF7F1 |
| Main text / secondary text | #252923 / #5F625B |
| Accent / accent hover | #176B5B / #105346 |
| Border / focus | #DED8CC / #176B5B, visible 2px outline plus offset |
| Success / warning / danger | #176344 / #805500 / #B42318, with text/icon and pale surfaces |
| Typeface | Segoe UI, system UI and sans-serif fallbacks. One family, weights 400/500/600/700; no new font provider |
...
These are the approved palette values. Full rendered accessibility verification remains an implementation gate. Input borders use #85887E; sidebar selection uses #E6EEE8. Verify every foreground/background pairing and state. Font files, if added, must be shipped locally with appropriate license and fallback metrics to limit layout shift.
...
The first shared design slice now includes a neutral canvas, dark slate text, one deep teal action color, restrained borders/shadows, system sans typography, a responsive navigation menu and consistent authentication/settings forms. Motion is limited to brief entrance/interaction transitions and disabled for prefers-reduced-motion. Inputs have explicit label/error associations. Native dialogs add focus wrapping, Escape dismissal and restoration.
...
## Approved concept and scope

The owner approved the off-white/teal mockup and explicitly requested implementation. Reference: [approved concept](docs/design/approved-m1-m2-concept.png). The structural proposal remains in [Module 1/2 proposal](docs/Design_Proposal_M1_M2.md); its original blue palette is superseded by this record. The archived prior design is [here](docs/design/Design-before-approved-refresh.md). Preserve true numeric salary inputs; image-generated dropdown arrows are not a feature requirement. Implementation evidence: [design verification](docs/design/Implementation.md).

## Module 2 implementation update

The library uses the shared shell and an accessible list, a focused upload dialog, persistent errors, original downloads, rename/delete actions and ready/review status. Review uses original source text beside a complete plain-text editor; mobile exposes Source/Edit views. No content is dropped to force heuristic fields. The detailed structured builder and formatted output remain M4. Reviewed saves are versioned, and stale saves fail with an explanation.

Settings tabs preserve their edits while switching views; navigation away, browser unload and sign-out protect unsaved work. Keeping the draft across tabs avoids a needless discard decision. This clarifies the original proposal’s view-switch wording. All page headings now share the shell’s left gutter; settings alone caps its form width.
